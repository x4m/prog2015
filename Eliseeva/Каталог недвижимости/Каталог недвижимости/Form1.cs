...
namespace Каталог_недвижимости
{
    ...
    {
        public DevExpress.XtraReports.UI.ReportPrintTool Rep;
        public Form1()
        {
            InitializeComponent();
        }
        ...
        private void печатьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != string.Empty && textBox2.Text != string.Empty)
            {
                switch (comboBox1.Text)
                {
                    case "Квартира":
                        {
                            Rep = new DevExpress.XtraReports.UI.ReportPrintTool(new FReport()
                            {
                                DataSource = new BindingSource()
                                {
                                    DataSource = GetDataFlat()
                                }
                            });
                            Rep.Report.CreateDocument(false);
                            Rep.ShowPreview();
                        }; break;
                    case "Дом":
                        {
                            Rep = new DevExpress.XtraReports.UI.ReportPrintTool(new HReport()
                            {
                                DataSource = new BindingSource()
                                {
                                    DataSource = GetDataHouse()
                                }
                            });
                            Rep.Report.CreateDocument(false);
                            Rep.ShowPreview();
                    ...
                    case "Земельный участок":
                        {
                            Rep = new DevExpress.XtraReports.UI.ReportPrintTool(new LReport()
                            {
                                DataSource = new BindingSource()
                                {
                                    DataSource = GetDataLand()
                                }
                            });
                            Rep.Report.CreateDocument(false);
                            Rep.ShowPreview();
                    ...
                    case "Коммерческая недвижимость":
                        {
                            Rep = new DevExpress.XtraReports.UI.ReportPrintTool(new LbReport()
                            {
                                DataSource = new BindingSource()
                                {
                                    DataSource = new Land() { Name = textBox1.Text, Phone = textBox2.Text, S = textBox7.Text, Far = textBox8.Text, Type = comboBox4.Text, Emid = checkBox1.Checked, Q = checkBox2.Checked, P = textBox6.Text, PForm = comboBox9.Text }
                                }
                            });
                            Rep.Report.CreateDocument(false);
                            Rep.ShowPreview();
        ...
        private Land GetDataLand()
        {
            return new Land() { Name = textBox1.Text, Phone = textBox2.Text, S = textBox7.Text, Far = textBox8.Text, Type = comboBox8.Text, Emid = checkBox1.Checked, Q = checkBox2.Checked, P = textBox6.Text, PForm = comboBox9.Text };
        }

        private house GetDataHouse()
        {
            return new house() { Name = textBox1.Text, Phone = textBox2.Text, S = textBox7.Text, Far = textBox3.Text, BuildingType = comboBox6.Text, Type = comboBox3.Text, Emid = checkBox1.Checked, Q = checkBox2.Checked, P = textBox6.Text, Floors = textBox5.Text, PForm = comboBox9.Text, Pool = checkBox7.Checked, Garage = checkBox5.Checked, Footer = checkBox6.Checked, Garden = checkBox8.Checked };
        }

        private Flat GetDataFlat()
        {
            return new Flat() { Name = textBox1.Text, Phone = textBox2.Text, Rooms = comboBox5.Text, S = textBox4.Text, Bal = checkBox3.Checked, PForm = comboBox9.Text, BuildingType = comboBox6.Text, P = textBox6.Text, Region = comboBox7.Text, Type = comboBox2.Text, Floor = textBox5.Text, Emid = checkBox1.Checked, Q = checkBox2.Checked };
        }
    }
}
